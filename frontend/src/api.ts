...
export function listMemories(view: "all" | "axioms" | "forgotten" | "expired" = "all"): Promise<Memory[]> {
...
export const searchMemories = (query: string, scope: SearchScope) => request<{ query: string; results: Memory[] }>("/search", { method: "POST", body: JSON.stringify({ query, limit: 100, scope, include_forgotten: scope === "remembering", include_expired: scope === "remembering" }) });
...
export const updateMemory = (id: string, payload: Partial<{ content: string; cadence: Cadence; base_importance: number; expires_at: string | null }>) => request<Memory>(`/memories/${id}`, { method: "PATCH", body: JSON.stringify(payload) });
